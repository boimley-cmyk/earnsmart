...
export default function Login() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simulate a failed login attempt since there's no backend
    setTimeout(() => {
      setIsLoading(false);
      setError("Incorrect user details. Please check your email and password.");
    }, 800);
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {error && (
              <motion.div 
                ...
                className="bg-red-50 border-2 border-red-200 text-red-600 p-4 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center gap-3"
              >
