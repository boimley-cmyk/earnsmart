  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[100] flex items-center justify-between px-8 py-5 bg-white border-b-2 border-dark">
      <Link to="/" className="flex items-center gap-3 group">
        <div className="w-10 h-10 bg-dark rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform brutal-shadow-sm">
          <img src="/logo.svg" alt="Earnie Logo" className="w-6 h-6" />
        </div>
        <span className="font-display text-3xl tracking-tighter text-dark">
      ...
      <div className="flex items-center gap-10">
        ...
        {isAuthenticated ? (
          ...
          <div className="flex items-center gap-6">
            ...
            <Link to="/signup" className="bg-brand text-dark border-2 border-dark px-6 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-dark hover:text-white transition-all brutal-shadow-sm">
              Sign Up
            </Link>
          </div>
        )}
